...
export default function AdminBanners() {
  ...
  const handleDrop = (e: React.DragEvent) => { e.preventDefault(); setDragging(false); const file = e.dataTransfer.files[0]; if (file) handleFile(file); };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.image_url) { setError('Faça upload de uma imagem primeiro'); return; }
    if (!form.title.trim()) { setError('Informe um título para o banner'); return; }
    setSaving(true); setError(null);
    try {
      await adminApi.post('/banners', { title: form.title.trim(), image_url: form.image_url, link_url: form.link_url.trim() || null, active: form.active, sort_order: banners.length });
      resetForm(); await loadBanners();
    } catch (err) { setError(err instanceof Error ? err.message : 'Erro ao salvar banner'); }
  ...
  const toggleActive = async (banner: Banner) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Erro ao atualizar banner'); }
  };
  ...
  const deleteBanner = async (banner: Banner) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Erro ao excluir banner'); }
  };
  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold text-white">Gerenciar Banners</h1>
          <p className="mt-1 text-sm text-neutral-400">Banners do catálogo — formato {BANNER_W} × {BANNER_H}px (16:6)</p>
        </div>
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 rounded-lg bg-neutral-100 px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-neutral-900 transition-colors hover:bg-white">
      ...
      {showForm && (
        <form onSubmit={handleSave} className={`${card} mb-8 p-6`}>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-bold text-white">Novo banner</h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mb-2 text-xs text-neutral-500">Tamanho recomendado: {BANNER_W} × {BANNER_H}px · Formato: JPG, PNG ou WebP · Máx 10MB</p>
              {!form.image_url ? (
                <div
                ...
                <div className="space-y-3">
                  ...
                  {imageInfo && (
                    <p className={`text-xs ${aspectRatioWarn ? 'text-amber-400' : 'text-neutral-500'}`}>
                      Dimensões: {imageInfo.width} × {imageInfo.height}px{aspectRatioWarn && ' — proporção diferente de 16:6. A imagem será recortada para preencher a área.'}
                    </p>
                  )}
              ...
              <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => { if (e.target.files?.[0]) handleFile(e.target.files[0]); e.target.value = ''; }} />
            </div>
            <div>
              <label className="flex items-center gap-2">
                ...
                <span className="text-sm text-neutral-300">Ativar banner imediatamente</span>
              </label>
            </div>
            <div className="flex gap-3">
              <button type="submit" disabled={saving || uploading || !form.image_url} className="inline-flex items-center gap-2 rounded-lg bg-neutral-100 px-6 py-3 text-xs font-bold uppercase tracking-widest text-neutral-900 transition-colors hover:bg-white disabled:opacity-50">
                {saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} {saving ? 'Salvando...' : 'Salvar banner'}
              </button>
      ...
      {banners.length === 0 ? (
        <div className={`${card} p-12 text-center`}>
          ...
          <p className="text-xs text-neutral-600">Clique em "Novo banner" para adicionar.</p>
        </div>
      ) : (
        <div className="space-y-4">
